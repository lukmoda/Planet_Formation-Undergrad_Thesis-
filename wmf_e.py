...
for j in range (0,6):
	...
	e_emb = np.zeros(m/2)				#array of embryo ecc
	mass_plan = np.zeros(p/2) 			#array of planetesimal masses
	a_plan = np.zeros(p/2) 				#array of planetesimal semi-major axis
	e_plan = np.zeros(p/2) 				#array of planetesimal ecc
	wmf_emb = np.zeros(m/2) 			#water mass fraction of embryos
	wmf_plan = np.zeros(p/2) 			#water mass fraction of planetesimals
	label_emb = np.zeros(m/2)			#color labels array for embryos
	label_plan = np.zeros(p/2)			#color labels array for planetesimals
	EMa = np.zeros(m/2) 					#array of semi-major axis at snapchot time (embryos)
	EMe = np.zeros(m/2) 					#array of ecc at snapchot time (embryos)
	EMm = np.zeros(m/2)					#array of mass at snapchot time (embryos)
	Pa = np.zeros(p/2) 					#array of semi-major axis at snapchot time (planetesimals)
	Pe = np.zeros(p/2) 					#array of ecc at snapchot time (planetesimals)
	...
	if j == 0:
		...
		ax = plt.gca()
		ax.autoscale_view()
		scale = mass_emb**(1./3.)*15000
		scale_plan = mass_plan**(1./3.)*3000
		left = .05 
		bottom = .05
		right = 1-left 
		top = 1-bottom 
		...
		axes[2,0].set_ylabel('Eccentricity')
		...
		ax1.scatter(a_emb,e_emb, s=scale, edgecolors='none', c=label_emb, cmap=cmap)
		ax1.scatter(a_plan,e_plan, s=scale_plan, edgecolors='none', c=label_plan, cmap=cmap)
	...
	#getting current semi-major axis, ecc and mass for each embryo at snapchot time

	if j !=0:
		for i in range (1,m/2+1):
			EM = np.genfromtxt('EM' + str(i) + '.aei', usecols=(1,2,7), skip_header=4)            #opening files and setting 
		...
		for i in range (1,p/2+1):
			P = np.genfromtxt('P' + str(i) + '.aei', usecols=(1,2,7), skip_header=4)            #opening files and setting 
			...
			if v >ltime:
				...
				Pe[i-1] = 0.
				Pm[i-1] = 0.
			if Pa[i-1] >= 5.:      #excluding planetesimals further than 5 AU
				Pa[i-1] = np.nan
		...
		scale_plan = Pm**(1./3.)*3000
			  		
		#plotting

		if j == 1:
			cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", (1,0.4,0.4), (1,0.5,0), "yellow", (0.25,1,0.5), "green", (0.4, 0.8, 1), "blue"])
			ax2.scatter(EMa,EMe, s=scale, edgecolors='none', c=label_emb, cmap=cmap)
			ax2.scatter(Pa,Pe, s=scale_plan, edgecolors='none', c=label_plan, cmap=cmap)
			...
			cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", (1,0.4,0.4), (1,0.5,0), "yellow", (0.25,1,0.5), "green", (0.4, 0.8, 1), "blue"])
			ax3.scatter(EMa,EMe, s=scale, edgecolors='none', c=label_emb, cmap=cmap)
			ax3.scatter(Pa,Pe, s=scale_plan, edgecolors='none', c=label_plan, cmap=cmap)
			...
			cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", (1,0.4,0.4), (1,0.5,0), "yellow", (0.25,1,0.5), "green", (0.4, 0.8, 1), "blue"])
			ax4.scatter(EMa,EMe, s=scale, edgecolors='none', c=label_emb, cmap=cmap)
			ax4.scatter(Pa,Pe, s=scale_plan, edgecolors='none', c=label_plan, cmap=cmap)
			...
			cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", (1,0.4,0.4), (1,0.5,0), "yellow", (0.25,1,0.5), "green", (0.4, 0.8, 1), "blue"])
			ax5.scatter(EMa,EMe, s=scale, edgecolors='none', c=label_emb, cmap=cmap)
			ax5.scatter(Pa,Pe, s=scale_plan, edgecolors='none', c=label_plan, cmap=cmap)
			...
			cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["red", (1,0.4,0.4), (1,0.5,0), "yellow", (0.25,1,0.5), "green", (0.4, 0.8, 1), "blue"])
			cl = ax6.scatter(EMa,EMe, s=scale, edgecolors='none', c=label_emb, cmap=cmap)
			ax6.scatter(Pa,Pe, s=scale_plan, edgecolors='none', c=label_plan, cmap=cmap)
			...
			for i in range (0, m/2):			
				if math.isnan(EMm[i]) == False:			
					final.write ('\nEM' + str(i) + ' = ' + "{0:.2f}".format(EMm[i] * 332946.) + ' Earth Mass at ' + "{0:.2f}".format(EMa[i]) + ' AU' + ' with wmf of ' + "{0:.5f}".format(wmf_emb[i]) + ' and ecc ' + "{0:.3f}".format(EMe[i]) + '\n') 
